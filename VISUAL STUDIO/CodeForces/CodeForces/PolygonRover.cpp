...
bool isInRange(V<ll> a) {
	...
	int p = lower_bound(IT(bounds), min3(a[0], a[1], a[2]), [](pair<V<ll>, V<ll>> a, ll minT) { // binary search
		//cout << "lower - ";
		//DPoints(a.first);
		//cout << "upper - ";
		//DPoints(a.second);
		//cout << "binary - " << minAll(a) << " <= " << minT << endl;
		return !(minAll(a) <= minT);
	}) - bounds.begin();
	if (p < bounds.size()) {
	//	cout << "found lower at - " << p << " : " << bounds.size() << endl;

		while (p < bounds.size() && minAll(bounds[p]) <= min3(a[0], a[1], a[2])) {
			auto i = bounds[p];
...
void solve() {
	...
	for (auto& i : bounds) { // swap upper and lower bounds
		if (min3(i.first[0], i.first[1], i.first[2]) > min3(i.second[0], i.second[1], i.second[2])) {
			swap(i.first, i.second);
		}
	}

	sort(IT(bounds), [](pair<V<ll>, V<ll>> a, pair<V<ll>, V<ll>> b) { // takes min of everything and sorts it by that
		return minAll(a) < minAll(b);
	});
